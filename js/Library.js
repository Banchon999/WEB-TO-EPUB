...
class Library { // eslint-disable-line no-unused-vars
    ...
    static async LibRenderSavedEpubs() {
        ...
        if (ShowAdvancedOptions) {
            ...
            LibRenderString += "<button id='libdownloadallzip'>⬇ Download All EPUBs (.zip)</button>";
        ...
        if ( ShowCompactView && !ShowAdvancedOptions) {
            ...
            if (ShowAdvancedOptions) {
                ...
                document.getElementById("libdownloadallzip").addEventListener("click", function() {Library.LibDownloadAllEpubsAsZip();});
    ...
    static LibDownloadAllEpubsAsZip() {
        Library.LibShowLoadingText();
        chrome.storage.local.get(null, async function(items) {
            let CurrentLibKeys = items["LibArray"];
            if (!CurrentLibKeys || CurrentLibKeys.length === 0) {
                Library.LibRenderSavedEpubs();
                return;
            }

            let zipFileWriter = new zip.BlobWriter("application/zip");
            // compressionMethod: 0 = Store (no re-compression, EPUBs are already compressed)
            let zipWriter = new zip.ZipWriter(zipFileWriter, {useWebWorkers: false, compressionMethod: 0});

            for (let i = 0; i < CurrentLibKeys.length; i++) {
                let epubBase64 = items["LibEpub" + CurrentLibKeys[i]];
                let rawName = items["LibFilename" + CurrentLibKeys[i]] || ("book_" + i);
                let filename = EpubPacker.addExtensionIfMissing(rawName);
                // Sanitise filename (strip illegal chars for zip entry names)
                filename = filename.replace(/[\\/:*?"<>|]/g, "_");
                zipWriter.add(filename, new zip.Data64URIReader(epubBase64));
            }

            let zipBlob = await zipWriter.close();
            let date = new Date().toISOString().slice(0, 10);
            Download.save(zipBlob, "Library_" + date + ".zip")
